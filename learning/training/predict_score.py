...
class ScorePredictor:
  def __init__(self, amp=True, weights_dir=None):
    self.amp = amp
    self.run_name = "2024-01-11-20-02-45"

    model_name = 'model_best.pth'
    code_dir = os.path.dirname(os.path.realpath(__file__))
    weights_dir = os.path.abspath(weights_dir) if weights_dir else f'{code_dir}/../../weights/{self.run_name}'
    ckpt_dir = os.path.join(weights_dir, model_name)

    self.cfg = OmegaConf.load(os.path.join(weights_dir, 'config.yml'))
